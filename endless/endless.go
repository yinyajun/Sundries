...
*/
func NewServer(addr string, s server) (srv *endlessServer) {
	...
	case *http.Server:
		srv.Server.(*http.Server).ReadTimeout = DefaultReadTimeOut
		srv.Server.(*http.Server).WriteTimeout = DefaultWriteTimeOut
		srv.Server.(*http.Server).MaxHeaderBytes = DefaultMaxHeaderBytes
	case *grpc.Server:
	}
...
func (srv *endlessServer) ListenAndServeTLS(certFile, keyFile string) (err error) {
	...
	if srv.Server.(*http.Server).TLSConfig != nil {
		*config = *srv.Server.(*http.Server).TLSConfig
	}
...
func (srv *endlessServer) shutdown() {
	...
	switch srv.Server.(type) {
	case *http.Server:
		srv.Server.(*http.Server).SetKeepAlivesEnabled(false)
		err := srv.EndlessListener.Close()
		if err != nil {
			log.Println(syscall.Getpid(), "EndlessListener.Close() error:", err)
		} else {
			log.Println(syscall.Getpid(), srv.EndlessListener.Addr(), "EndlessListener closed.")
		}
	case *grpc.Server:
		srv.Server.(*grpc.Server).GracefulStop()
		log.Println(syscall.Getpid(), srv.EndlessListener.Addr(), "EndlessListener closed.")
	}
}
