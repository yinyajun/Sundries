# Refactor to support GRPC server

Original endless is great but only supports httpServer. Now it can apply for both httpServer and grpc server.
**Only endless.go is modified**
...
endless.DefaultReadTimeOut = time.Duration(20) * time.Second
endless.DefaultWriteTimeOut = time.Duration(20) * time.Second
server := endless.NewServer(addr, s)
...
pb.RegisterHelloServer(server, new(services.HelloService))
server := endless.NewServer(addr, s)
err := server.ListenAndServe()
```
...
## About Refactor(Support gRPC server)
Original endless is great but only supports httpServer. Now it can apply for both httpServer and grpc server.
