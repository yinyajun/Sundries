...
# image_lists参数给出了所有图片信息。就是result的数据结构
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    if not os.path.exists(bottleneck_path):
        ...
        # 获取图片内容。读出来的是没有解码的原始图像byte。
        image_data = gfile.FastGFile(image_path, 'rb').read()
        # 由于输入的图片大小不一致，此处得到的image_data大小也不一致（已验证）
...
def get_random_cached_bottlenecks(sess, n_classes, image_lists, how_many, category,
    ...
    for _ in range(how_many):
        # 随机一个类别和图片的编号加入当前的训练数据。（随机选择字典的一个key）
