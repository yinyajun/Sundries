...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    # 如果这个特征向量文件不存在，则通过Inception-v3模型来计算特征向量，并将计算的结果存入文件。(这个文件可以作为视觉特征给其他模型使用)
...
def main(_):
    ...
    # 定义一层全连接层来解决新的图片分类问题（可以加上正则，偷懒省略）。
    ...
    train_step = tf.train.AdagradOptimizer(LEARNING_RATE).minimize(cross_entropy_mean)

    # 分类指标
    with tf.name_scope('evaluation'):
        preds = tf.argmax(final_tensor, 1)  # (m,)
        labels = tf.argmax(ground_truth_input, 1)  # (m,)
        # true positive:
        with tf.variable_scope('true_positive'):
            is_tp = tf.logical_and(tf.equal(preds, 1), tf.equal(labels, 1))
            tp = tf.reduce_sum(tf.cast(is_tp, tf.float32))
        # false positive
        with tf.variable_scope('false_positive'):
            is_fp = tf.logical_and(tf.equal(preds, 1), tf.equal(labels, 0))
            fp = tf.reduce_sum(tf.cast(is_fp, tf.float32))
        # false negative
        with tf.variable_scope('false_negative'):
            is_fn = tf.logical_and(tf.equal(preds, 0), tf.equal(labels, 1))
            fn = tf.reduce_sum(tf.cast(is_fn, tf.float32))
        # true negative
        with tf.variable_scope('true_negative'):
            is_tn = tf.logical_and(tf.equal(preds, 0), tf.equal(labels, 0))
            tn = tf.reduce_sum(tf.cast(is_tn, tf.float32))
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            _, train_loss = sess.run([train_step, cross_entropy_mean],
                                     feed_dict={bottleneck_input: train_bottlenecks,
                                                ground_truth_input: train_ground_truth})
            # 在验证集上测试正确率。
            if i % 50 == 0 or i + 1 == STEPS:
                validation_bottlenecks, validation_ground_truth = get_random_cached_bottlenecks(
                    sess, n_classes, image_lists, VAL_BATCH, 'validation', jpeg_data_tensor, bottleneck_tensor)
                val_acc, val_loss = sess.run([evaluation_step, cross_entropy_mean],
                                             feed_dict={bottleneck_input: validation_bottlenecks,
                                                        ground_truth_input: validation_ground_truth})
                train_loss_history.append(train_loss)
                val_loss_history.append(val_loss)
                print('Step %d: Val acc on random sampled %d examples = %.1f%%, val loss: %0.4f, train loss: %0.4f'
                      % (i, VAL_BATCH, val_acc * 100, val_loss, train_loss))
            if i % 2000 == 0:
                w, b = sess.run([weights, biases])
                np.savez('./transfer_coef_%s.npz' % (str(i).zfill(6)), w=w, b=b)

        # 在最后的测试数据上测试正确率
        test_bottlenecks, test_ground_truth = get_test_bottlenecks(sess, image_lists, n_classes,
                                                                   jpeg_data_tensor, bottleneck_tensor)
        test_acc, a, b, c = sess.run([evaluation_step, acc, prec, recall],
                                     feed_dict={bottleneck_input: test_bottlenecks,
                                                ground_truth_input: test_ground_truth})
        print('Final test accuracy = %.1f%%, acc = %f, prec = %f, recall = %f' % (test_acc * 100, a, b, c))
        # 可视化学习曲线使用
        print("val_loss:", val_loss_history)
        print("train_loss:", train_loss_history)
        # 保存LR参数
        w, b = sess.run([weights, biases])
        np.savez('./transfer_coef.npz', w=w, b=b)
