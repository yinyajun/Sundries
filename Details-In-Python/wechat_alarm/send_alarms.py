...
contents = """一个人早起晚睡不难，但在一群晚起早睡的人中早起晚睡不简单。
...
def send_alarms(friend_name, contents):
    try:
        target_friend = bot.friends().search(friend_name)[0]
        target_friend.send(contents)
        t = Timer(84600, send_alarms)
        t.start()
    except:
        pass
